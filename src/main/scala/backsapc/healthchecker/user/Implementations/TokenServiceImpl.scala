...
class TokenServiceImpl(repository: AccountRepository, bCrypt: AsyncBcrypt)(
  ...
  def generate(request: LoginRequest): Future[GenerateResult] =
    repository.getByLogin(request.login).flatMap {
      case Some(account) =>
        bCrypt.verify(request.password, account.password).map {
          case true =>
            val claims =
              setClaims(account.id, request.login, tokenExpiryPeriodInDays)
