...
class InMemoryAccountRepository extends AccountRepository {
  ...
  override def getByLogin(login: String): Future[Option[Account]] = Future.successful(
    repo.asScala.find(p => p._2.login == login) map (_._2)
  )

  override def updatePassword(id: UUID, password: String): Future[Account] = Future.successful {
    val account = repo.asScala.getOrElse(id, throw new NoSuchElementException)
    repo.replace(id, account.copy(password = password))
  }

  override def existsWithId(accountId: UUID): Future[Boolean] = Future.successful(repo.asScala.contains(accountId))

  override def existsWithLogin(login: String): Future[Boolean] = Future.successful(repo.asScala.exists(_._2.login == login))

  override def existsWithEmail(email: String): Future[Boolean] = Future.successful(repo.asScala.exists(_._2.email == email))
}
